...
### **Algorithm for Go-Back-N Sliding Window Protocol (Server-Side):**  

1. **Initialize Connection:**  
   - Create a socket.  
   - Bind it to a specific port.  
   - Listen for incoming client connections and accept one.  

2. **Send Messages with Go-Back-N (n=3):**  
   - Start sending messages (`i = 0 to 9`).  
   - Use a timeout mechanism (`select()`) to wait for acknowledgments.  

3. **Handle Timeouts:**  
   - If an acknowledgment is not received within the timeout:  
     - Retransmit the last **n** (3) messages.  
     - Restart the process from the last acknowledged message.  

4. **Receive and Process Acknowledgments:**  
   - If acknowledgment is received, continue sending the next message.  

5. **Close Connection:**  
   - Once all messages are acknowledged, close the socket.
   */
