...
### **Algorithm for Go-Back-N Sliding Window Protocol (Client-Side):**  
...
2. **Data Transmission:**  
   - For each frame (`i = 0` to `9`):  
     - Read the received message from the server.  
     - If the received frame is out of order, discard it and retransmit.  
     - Otherwise, send an acknowledgment (`ACK`) for the received frame.  
     - Simulate packet loss for a specific frame (`i = 8`).  

3. **Close the Connection:**  
   - Terminate the socket after transmission.
