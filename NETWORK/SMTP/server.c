...
1. **Initialize and bind socket:**  
   - Create a UDP/TCP socket.  
   - Bind it to the given port.  

2. **Wait for client request:**  
   - Receive the initial SMTP request from the client.  
   - Respond with `220 <server_name>`.  

3. **Handle HELO command:**  
   - Receive `HELO <client_name>`.  
   - Respond with `250 Hello <client_name>`.  

4. **Handle sender's email:**  
   - Receive `MAIL FROM: <sender_email>`.  
   - Respond with `250 Sender OK`.  

5. **Handle recipient's email:**  
   - Receive `RCPT TO: <recipient_email>`.  
   - Respond with `250 Recipient OK`.  

6. **Receive email content:**  
   - Receive `DATA` command and respond with `354 Enter mail, end with "."`.  
   - Read email lines until a single `.` is received.  
   - Confirm message delivery with `250 Message accepted`.  

7. **Close connection:**  
   - Receive `QUIT` command.  
   - Respond with `221 Server closing connection`.  
   - Close the socket.  

Let me know if you need a **TCP-based version** or authentication! 🚀
*/
