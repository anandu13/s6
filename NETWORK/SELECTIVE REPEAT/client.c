...
### **Algorithm for Selective Repeat Sliding Window Protocol (Client-Side):**  

1. **Initialize Connection:**  
   - Create a socket and connect to the server.  

2. **Receive Messages from Server:**  
   - Loop until all frames (0 to 8) are received.  
   - Read a message from the server.  
   - Simulate packet corruption using a random function.  

3. **Send Acknowledgment:**  
   - If the packet is **not corrupted**, send a positive acknowledgment (`ACK`).  
   - If the packet is **corrupted**, send a negative acknowledgment (`NAK`).  
   - Simulate loss of a specific packet (`count == 7`).  

4. **Repeat Until All Frames Are Acknowledged.**  
