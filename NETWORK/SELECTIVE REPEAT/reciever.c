...
### **Algorithm for Selective Repeat Sliding Window Protocol (Server-Side):**  

1. **Initialize Connection:**  
   - Create a socket and bind it to a port.  
   - Listen for incoming client connections.  

2. **Send Frames in a Sliding Window (Size = 3):**  
   - Loop through the frames and send them in batches of 3.  

3. **Wait for Acknowledgment:**  
   - Use `select()` with a timeout to wait for acknowledgments.  
   - If timeout occurs, **resend** the corresponding frame.  

4. **Handle Corrupted Frames:**  
   - If the client sends a **negative acknowledgment (NAK)**, **resend** the corresponding frame.  
